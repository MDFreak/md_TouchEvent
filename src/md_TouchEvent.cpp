...
void md_TouchEvent::pollTouchScreen()
  {
    TS_Point p,scr;
    boolean tch;
    int16_t xr,yr,dx,dy;
    ...
    if (tch && _down)
      { //continous touch
        ...
        _last = p;
        if (_drawMode && ((dx > _moveth) || (dy > _moveth)))
          { //movement?
...
void md_TouchEvent::setResolution(int16_t xResolution, int16_t yResolution)
  {
    _xResolution = xResolution;
    _yResolution = yResolution;
  }

void md_TouchEvent::setDrawMode(boolean drawMode)
  {
    _drawMode = drawMode;
  }


void md_TouchEvent::calibrate(uint16_t xMin, uint16_t yMin, uint16_t xMax, uint16_t yMax)
  {
    _tsMinX = xMin;
    _tsMinY = yMin;
    _tsMaxX = xMax;
...
void md_TouchEvent::setMoveTreshold(uint16_t threshold)
  {
    _moveth = threshold;
  }

void md_TouchEvent::setSwipe(uint16_t swipeX, uint16_t swipeY)
  {
    _swipeX = swipeX;
    _swipeY = swipeY;
  }

void md_TouchEvent::setLongClick(uint16_t clickLong)
  {
    _clickLong = clickLong;
  }

void md_TouchEvent::setDblClick(uint16_t dblclick)
  {
    _dblClick = dblclick;
  }

void md_TouchEvent::registerOnTouchDown(void (*callback)(TS_Point p))
  {
    _onTouchDown = callback;
  }
void md_TouchEvent::registerOnTouchUp(void (*callback)(TS_Point p))
  {
    _onTouchUp = callback;
  }
void md_TouchEvent::registerOnTouchClick(void (*callback)(TS_Point p))
  {
    _onTouchClick = callback;
  }
void md_TouchEvent::registerOnTouchDblClick(void (*callback)(TS_Point p))
  {
    _onTouchDblClick = callback;
  }
void md_TouchEvent::registerOnTouchLong(void (*callback)(TS_Point p))
  {
    _onTouchLong = callback;
  }
void md_TouchEvent::registerOnTouchDraw(void (*callback)(TS_Point p))
  {
    _onTouchDraw = callback;
  }
void md_TouchEvent::registerOnTouchSwipe(void (*callback)(uint8_t direction))
  {
    _onTouchSwipe = callback;
  }

void md_TouchEvent::registerOnAllEvents(void (*callback)(int16_t x,int16_t y,EV event))
  {
    _onAllEvents = callback;
  }

boolean md_TouchEvent::isInArea(TS_Point p, int16_t x1, int16_t y1, int16_t x2, int16_t y2)
  {
    int16_t tmp;
    if (x1 > x2)
      {
...
uint8_t md_TouchEvent::swipeDirection(TS_Point b, TS_Point e)
  {
    TS_Point ptmp;
    int bx,by,ex,ey;
...
TS_Point md_TouchEvent::toScreen(TS_Point p)
  {
    TS_Point p1;
    if (abs(_xResolution) > abs(_yResolution))
...
void md_TouchEvent::autocalibrate(uint8_t which)
  {
    _autoWhich = which;
  }
//read the min and max raw values
void md_TouchEvent::getMinMax(uint16_t * xmin, uint16_t * ymin, uint16_t * xmax, uint16_t * ymax)
  {
    *xmin = _tsMinX;
    *ymin = _tsMinY;
    *xmax = _tsMaxX;
